...
if __name__ == "__main__":
    ...
    try:
        ...
        for venta in ventas[:5]:  # Mostrar solo las primeras 5 ventas
            print(f"ID: {venta['id_venta']}, Código: {venta['codigo_venta']}, "
                  f"Cliente: {venta['cliente_nombre']}, Total: {venta['total']}")
            
        estados = gestor.obtener_estados_venta()
        print(f"\nTotal de estados de venta: {len(estados)}")
        for estado in estados:
            print(f"ID: {estado['id_estado']}, Nombre: {estado['nombre']}")
            
        variedades = gestor.obtener_variedades_disponibles()
        print(f"\nTotal de variedades disponibles: {len(variedades)}")
        for variedad in variedades[:5]:  # Mostrar solo las primeras 5 variedades
            print(f"ID: {variedad['id_variedad']}, Variedad: {variedad['variedad']}, "
                  f"Tipo: {variedad['tipo_cultivo']}")
    except Exception as e:
